...
class OctaveConv(nn.Module):
        ...
        if x_l is not None:
            ...
            if self.is_dw:
                ...
                if x_l2h.shape[2:] != x_h2h.shape[2:]:
                    x_l2h = F.interpolate(x_l2h,x_h2h.shape[2:], mode="bilinear")
                x_h = x_l2h + x_h2h

                if x_h2l is not None and x_l2l is not None:
                    if x_h2l.shape[2:] != x_l2l.shape[2:]:
                        x_h2l = F.interpolate(x_h2l, x_l2l.shape[2:], mode="bilinear")
                    x_l = x_h2l + x_l2l
                else:
                    x_l = None
    ...
    def forward(self, x):
        x_h, x_l = self._forward(x)
        if self.res:
            x_r_h, x_r_l = x if type(x) is tuple else (x, None)
            x_h = x_h + x_r_h
            x_l = x_l + x_r_l
        return self.act(x_h), self.act(x_l)
